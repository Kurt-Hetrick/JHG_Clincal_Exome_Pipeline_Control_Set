...
START_APPLY_RECALIBRATION_SNP=`date '+%s'` # capture time process starts for wall clock tracking purposes.
	...
	# construct command line
		...
		CMD=$CMD" -T ApplyRecalibration" \
			...
			CMD=$CMD" --input:VCF $CORE_PATH/$PROJECT/TEMP/CONTROL_DATA_SET.VQSR.INDEL.vcf" \
			...
			CMD=$CMD" -o $CORE_PATH/$PROJECT/TEMP/CONTROL_DATA_SET.VQSR.vcf" \
...
END_APPLY_RECALIBRATION_SNP=`date '+%s'` # capture time process ends for wall clock tracking purposes.
